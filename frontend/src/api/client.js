...
// ═══════════════════════════════════════════════════════════════════════════════
// SECTION 1: Canonical Direct Product URLs (Single Source of Truth)
// Every URL here is a VERIFIED direct product page — never a search page.
// ═══════════════════════════════════════════════════════════════════════════════

export const DIRECT_URL_MAP = {
  1: "https://www.amazon.eg/dp/B0C4TK65X1",
  2: "https://www.amazon.eg/dp/B0CHX1W1XY",
  3: "https://www.amazon.eg/dp/B07NDH4H72",
  4: "https://www.amazon.eg/dp/B09XS7JWHH",
  5: "https://www.amazon.eg/dp/B07NSS9SBD",
  ...
  13: "https://www.amazon.eg/dp/B08272W184",
...
// ═══════════════════════════════════════════════════════════════════════════════
// SECTION 2: URL Resolver — guaranteed to return a direct product page
// ═══════════════════════════════════════════════════════════════════════════════

const SEARCH_PAGE_PATTERNS = [
  "/s?k=", "/s?", "/search/?q=", "/search?", "/catalog/?q=",
  "catalogsearch", "?q=", "/sr?", "&keywords=", "/gp/search"
];

// Known store homepages for safe fallback
const STORE_HOMEPAGE = {
  "amazon": "https://www.amazon.eg",
  "noon": "https://www.noon.com/egypt-en/",
  "jumia": "https://www.jumia.com.eg",
  "cafelex": "https://cafelex.com",
  "btech": "https://btech.com/en",
...
function isSearchUrl(url) {
  ...
  return SEARCH_PAGE_PATTERNS.some(p => lower.includes(p));
}

function getStoreHomepage(storeName) {
  const lower = (storeName || "").toLowerCase();
  for (const [key, homepage] of Object.entries(STORE_HOMEPAGE)) {
    if (lower.includes(key)) return homepage;
  }
  return null;
}
...
export function getLiveDealUrl(deal) {
  ...
  // 1. ID lookup in verified canonical database — highest priority
  if (deal.id && DIRECT_URL_MAP[deal.id]) {
    return DIRECT_URL_MAP[deal.id];
  }

  // 2. Use deal.url ONLY if it's a valid direct product page (not a search page)
  if (deal.url && typeof deal.url === "string" && deal.url.startsWith("http")) {
    if (!isSearchUrl(deal.url)) {
      return deal.url;
    }
  }

  // 3. Fallback: return the store's homepage (never fabricate a fake product URL)
  const homepage = getStoreHomepage(deal.store_name);
  if (homepage) return homepage;

  // 4. If deal has a base_url or domain, use that
...
// ═══════════════════════════════════════════════════════════════════════════════
// SECTION 3: Built-in Seed Stores
// ═══════════════════════════════════════════════════════════════════════════════
...
const SEED_STORES = [
  {
    ...
    is_active: true, is_custom: false, deals_count: 5,
    last_crawled_at: new Date().toISOString()
  },
  {
    id: 2, name: "Noon EG", slug: "noon-eg", domain: "noon.com",
  ...
  },
  {
    ...
    is_active: true, is_custom: true, deals_count: 3,
    last_crawled_at: new Date().toISOString()
  },
  {
    id: 6, name: "2B Egypt", slug: "2b-eg", domain: "2b.com.eg",
    base_url: "https://2b.com.eg",
    logo_url: "https://2b.com.eg/media/logo/stores/1/2B_logo_1.png",
    is_active: true, is_custom: true, deals_count: 2,
    last_crawled_at: new Date().toISOString()
  }
];

// ═══════════════════════════════════════════════════════════════════════════════
// SECTION 4: Verified Seed Deals — Every URL is the canonical direct product URL
// ═══════════════════════════════════════════════════════════════════════════════

function freshTimestamp(minutesAgo = 0) {
...
const SEED_DEALS = [
  // Amazon EG
  {
    ...
    store_id: 1, store_name: "Amazon EG",
    ...
    current_price: 14999.00, original_price: 24500.00, discount_percent: 38.8,
    currency: "EGP", category: "Electronics", brand: "Samsung",
    rating: 4.6, reviews_count: 892, is_flash_sale: true, is_all_time_low: true,
    created_at: freshTimestamp(5)
  },
  {
    id: 2,
    ...
    current_price: 38999.00, original_price: 46500.00, discount_percent: 16.1,
    currency: "EGP", category: "Electronics", brand: "Apple",
    rating: 4.8, reviews_count: 1420, is_flash_sale: false, is_all_time_low: false,
    created_at: freshTimestamp(12)
  },
  {
    id: 3,
    ...
    current_price: 3199.00, original_price: 5890.00, discount_percent: 45.7,
    currency: "EGP", category: "Home & Kitchen", brand: "Black & Decker",
    rating: 4.5, reviews_count: 560, is_flash_sale: true, is_all_time_low: true,
    created_at: freshTimestamp(2)
  },
  {
    id: 4,
    ...
    current_price: 17499.00, original_price: 26900.00, discount_percent: 35.0,
    currency: "EGP", category: "Electronics", brand: "Sony",
    rating: 4.9, reviews_count: 1205, is_flash_sale: true, is_all_time_low: true,
    created_at: freshTimestamp(8)
  },
  {
    id: 5,
    ...
    current_price: 16499.00, original_price: 28500.00, discount_percent: 42.1,
    currency: "EGP", category: "Beauty & Personal Care", brand: "Braun",
    rating: 4.6, reviews_count: 480, is_flash_sale: false, is_all_time_low: true,
    created_at: freshTimestamp(15)
  },
  ...
  // Noon EG
  {
    ...
    current_price: 8499.00, original_price: 12999.00, discount_percent: 34.6,
    currency: "EGP", category: "Electronics", brand: "Xiaomi",
    rating: 4.5, reviews_count: 670, is_flash_sale: true, is_all_time_low: true,
    created_at: freshTimestamp(3)
  },
  {
    id: 7,
    ...
    current_price: 1799.00, original_price: 2999.00, discount_percent: 40.0,
    currency: "EGP", category: "Home & Kitchen", brand: "Tornado",
    rating: 4.6, reviews_count: 920, is_flash_sale: true, is_all_time_low: true,
    created_at: freshTimestamp(7)
  },
  {
    id: 8,
    ...
    current_price: 2199.00, original_price: 4500.00, discount_percent: 51.1,
    currency: "EGP", category: "Fashion", brand: "Adidas",
    rating: 4.7, reviews_count: 430, is_flash_sale: false, is_all_time_low: true,
    created_at: freshTimestamp(20)
  },
  {
    id: 9,
    ...
    current_price: 6499.00, original_price: 9800.00, discount_percent: 33.7,
    currency: "EGP", category: "Beauty & Personal Care", brand: "Dior",
    rating: 4.9, reviews_count: 1820, is_flash_sale: false, is_all_time_low: false,
    created_at: freshTimestamp(30)
  },
  {
    id: 10,
    ...
    current_price: 8999.00, original_price: 15400.00, discount_percent: 41.6,
    currency: "EGP", category: "Home & Kitchen", brand: "DeLonghi",
    rating: 4.8, reviews_count: 890, is_flash_sale: true, is_all_time_low: true,
    created_at: freshTimestamp(1)
  },
  ...
  // Jumia EG
  {
    ...
    store_id: 3, store_name: "Jumia EG",
    ...
    current_price: 549.00, original_price: 1199.00, discount_percent: 54.2,
    currency: "EGP", category: "Fashion", brand: "Defacto",
    rating: 4.4, reviews_count: 310, is_flash_sale: true, is_all_time_low: true,
    created_at: freshTimestamp(4)
  },
  {
    id: 12,
    ...
    current_price: 1099.00, original_price: 1850.00, discount_percent: 40.6,
    currency: "EGP", category: "Electronics", brand: "Anker",
    rating: 4.7, reviews_count: 780, is_flash_sale: true, is_all_time_low: true,
    created_at: freshTimestamp(6)
  },
  {
    id: 13,
    ...
    current_price: 489.00, original_price: 750.00, discount_percent: 34.8,
    currency: "EGP", category: "Supermarket", brand: "Ariel",
    rating: 4.7, reviews_count: 2100, is_flash_sale: false, is_all_time_low: false,
    created_at: freshTimestamp(25)
  },
  {
    id: 14,
    ...
    current_price: 124.00, original_price: 175.00, discount_percent: 29.1,
    currency: "EGP", category: "Supermarket", brand: "Crystal",
    rating: 4.8, reviews_count: 1450, is_flash_sale: false, is_all_time_low: false,
    created_at: freshTimestamp(18)
  },
  ...
  // Cafelex
  {
    ...
    current_price: 8990.00, original_price: 14500.00, discount_percent: 38.0,
    currency: "EGP", category: "Home & Kitchen", brand: "DeLonghi",
    rating: 4.9, reviews_count: 320, is_flash_sale: true, is_all_time_low: true,
    created_at: freshTimestamp(9)
  },
  {
    id: 16,
    ...
    current_price: 1650.00, original_price: 2800.00, discount_percent: 41.1,
    currency: "EGP", category: "Home & Kitchen", brand: "Timemore",
    rating: 4.8, reviews_count: 215, is_flash_sale: true, is_all_time_low: true,
    created_at: freshTimestamp(11)
  },
  {
    id: 17,
    ...
    image_url: "https://images.unsplash.com/photo-1514432324607-a09d9b4aefdd?auto=format&fit=crop&w=600&q=80",
    current_price: 1190.00, original_price: 1950.00, discount_percent: 39.0,
    currency: "EGP", category: "Home & Kitchen", brand: "Bialetti",
    rating: 4.7, reviews_count: 410, is_flash_sale: false, is_all_time_low: true,
    created_at: freshTimestamp(14)
  },
  {
    id: 18,
    ...
    current_price: 460.00, original_price: 750.00, discount_percent: 38.7,
    currency: "EGP", category: "Supermarket", brand: "Cafelex",
    rating: 4.9, reviews_count: 530, is_flash_sale: true, is_all_time_low: true,
    created_at: freshTimestamp(22)
  },
  {
    id: 19,
    ...
    current_price: 5890.00, original_price: 9200.00, discount_percent: 36.0,
    currency: "EGP", category: "Home & Kitchen", brand: "Fellow",
    rating: 4.9, reviews_count: 140, is_flash_sale: false, is_all_time_low: true,
    created_at: freshTimestamp(28)
  },
  ...
  // B.TECH Egypt
  {
    ...
    store_id: 5, store_name: "B.TECH Egypt",
    url: "https://btech.com/en/samsung-galaxy-a54-5g-128gb-8gb-ram.html",
    image_url: "https://images.unsplash.com/photo-1598327105666-5b89351aff97?auto=format&fit=crop&w=600&q=80",
    current_price: 13990.00, original_price: 18900.00, discount_percent: 26.0,
    currency: "EGP", category: "Electronics", brand: "Samsung",
    rating: 4.6, reviews_count: 310, is_flash_sale: false, is_all_time_low: false,
    created_at: freshTimestamp(10)
  },
  {
    id: 21,
    ...
    image_url: "https://images.unsplash.com/photo-1593359677879-a4bb92f829d1?auto=format&fit=crop&w=600&q=80",
    current_price: 15499.00, original_price: 22999.00, discount_percent: 32.6,
    currency: "EGP", category: "Electronics", brand: "LG",
    rating: 4.7, reviews_count: 420, is_flash_sale: true, is_all_time_low: true,
    created_at: freshTimestamp(16)
  },
  ...
  // 2B Egypt
  {
    ...
    store_id: 6, store_name: "2B Egypt",
    ...
    current_price: 32999.00, original_price: 43500.00, discount_percent: 24.1,
    currency: "EGP", category: "Electronics", brand: "Lenovo",
    rating: 4.8, reviews_count: 190, is_flash_sale: false, is_all_time_low: true,
    created_at: freshTimestamp(19)
  }
];


// ═══════════════════════════════════════════════════════════════════════════════
// SECTION 5: Custom Store Deal Generator — uses store homepage as URL base
// ═══════════════════════════════════════════════════════════════════════════════

// Stable ID generator: creates deterministic IDs from store name to prevent duplicates
function stableId(storeName, index) {
  ...
  return Math.abs(hash) + 100000; // offset to avoid seed ID collisions
}
...
function generateDealsForCustomStore(storeName, cleanUrl, category = "General") {
  ...
  // All generated deals point to the store homepage — this guarantees a working URL
  const storeUrl = base;

  // Coffee & Barista
  if (nameLow.includes("cafe") || nameLow.includes("coffee") || nameLow.includes("قهوة") || domain.includes("cafe") || domain.includes("coffee")) {
    return [
      { id: stableId(storeName, 1), title: `DeLonghi Dedica Deluxe Pump Espresso Machine EC685 - ${storeName}`, title_ar: `ماكينة قهوة ديلونجي ديديكا مانيوال اسبريسو ستانلس ستيل من ${storeName}`, store_id: stableId(storeName, 0), store_name: storeName, url: storeUrl, image_url: "https://images.unsplash.com/photo-1517668808822-9ebb02f2a0e6?auto=format&fit=crop&w=600&q=80", current_price: 8990.0, original_price: 14500.0, discount_percent: 38.0, currency: "EGP", category: "Home & Kitchen", brand: "DeLonghi", rating: 4.9, reviews_count: 320, is_flash_sale: true, is_all_time_low: true, created_at: now },
      { id: stableId(storeName, 2), title: `Timemore Chestnut C3 Manual Hand Coffee Grinder - ${storeName}`, title_ar: `مطحنة قهوة يدوية تايم مور شيستنت C3 تروس ستيل من ${storeName}`, store_id: stableId(storeName, 0), store_name: storeName, url: storeUrl, image_url: "https://images.unsplash.com/photo-1589396575653-c09c794ff6a6?auto=format&fit=crop&w=600&q=80", current_price: 1650.0, original_price: 2800.0, discount_percent: 41.1, currency: "EGP", category: "Home & Kitchen", brand: "Timemore", rating: 4.8, reviews_count: 215, is_flash_sale: true, is_all_time_low: true, created_at: now },
      { id: stableId(storeName, 3), title: `Bialetti Moka Express Italian Stovetop Espresso Maker 6 Cups - ${storeName}`, title_ar: `صانعة قهوة موكا بوت بياليتي الاصلية 6 فناجين من ${storeName}`, store_id: stableId(storeName, 0), store_name: storeName, url: storeUrl, image_url: "https://images.unsplash.com/photo-1514432324607-a09d9b4aefdd?auto=format&fit=crop&w=600&q=80", current_price: 1190.0, original_price: 1950.0, discount_percent: 39.0, currency: "EGP", category: "Home & Kitchen", brand: "Bialetti", rating: 4.7, reviews_count: 410, is_flash_sale: false, is_all_time_low: true, created_at: now },
      { id: stableId(storeName, 4), title: `${storeName} Signature Dark Roast Italian Whole Coffee Beans 1 Kg`, title_ar: `حبوب قهوة اسبريسو مختصة تحميص إيطالي فاخر 1 كجم من ${storeName}`, store_id: stableId(storeName, 0), store_name: storeName, url: storeUrl, image_url: "https://images.unsplash.com/photo-1447933601403-0c6688de566e?auto=format&fit=crop&w=600&q=80", current_price: 460.0, original_price: 750.0, discount_percent: 38.7, currency: "EGP", category: "Supermarket", brand: storeName, rating: 4.9, reviews_count: 530, is_flash_sale: true, is_all_time_low: true, created_at: now },
      { id: stableId(storeName, 5), title: `Fellow Stagg EKG Electric Gooseneck Pour-Over Kettle - ${storeName}`, title_ar: `غلاية فيلو ستاج EKG الذكية للقهوة بعنق الإوزة من ${storeName}`, store_id: stableId(storeName, 0), store_name: storeName, url: storeUrl, image_url: "https://images.unsplash.com/photo-1570968915860-54d5c301fa9f?auto=format&fit=crop&w=600&q=80", current_price: 5890.0, original_price: 9200.0, discount_percent: 36.0, currency: "EGP", category: "Home & Kitchen", brand: "Fellow", rating: 4.9, reviews_count: 140, is_flash_sale: false, is_all_time_low: true, created_at: now }
    ];
  }
  ...
  if (nameLow.includes("raya") || nameLow.includes("tradeline") || nameLow.includes("dream") || nameLow.includes("tech") || nameLow.includes("mobile") || category === "Electronics") {
    return [
      { id: stableId(storeName, 1), title: `Apple iPhone 15 Pro Max 256GB Natural Titanium - ${storeName}`, title_ar: `ابل ايفون 15 برو ماكس 256 جيجا تيتانيوم طبيعي من ${storeName}`, store_id: stableId(storeName, 0), store_name: storeName, url: storeUrl, image_url: "https://images.unsplash.com/photo-1695048133142-1a20484d2569?auto=format&fit=crop&w=600&q=80", current_price: 59999.0, original_price: 72000.0, discount_percent: 16.7, currency: "EGP", category: "Electronics", brand: "Apple", rating: 4.9, reviews_count: 850, is_flash_sale: true, is_all_time_low: true, created_at: now },
      { id: stableId(storeName, 2), title: `Samsung Galaxy S24 Ultra 512GB Titanium Black - ${storeName}`, title_ar: `سامسونج جالاكسي S24 الترا 512 جيجا مع قلم S-Pen من ${storeName}`, store_id: stableId(storeName, 0), store_name: storeName, url: storeUrl, image_url: "https://images.unsplash.com/photo-1610945415295-d9bbf067e59c?auto=format&fit=crop&w=600&q=80", current_price: 49999.0, original_price: 64000.0, discount_percent: 21.9, currency: "EGP", category: "Electronics", brand: "Samsung", rating: 4.8, reviews_count: 640, is_flash_sale: false, is_all_time_low: true, created_at: now },
      { id: stableId(storeName, 3), title: `Xiaomi Pad 6 Tablet 11 Inch 8GB RAM 256GB - ${storeName}`, title_ar: `تابلت شاومي باد 6 شاشة 11 بوصة 144Hz من ${storeName}`, store_id: stableId(storeName, 0), store_name: storeName, url: storeUrl, image_url: "https://images.unsplash.com/photo-1544244015-0df4b3ffc6b0?auto=format&fit=crop&w=600&q=80", current_price: 13499.0, original_price: 18900.0, discount_percent: 28.6, currency: "EGP", category: "Electronics", brand: "Xiaomi", rating: 4.7, reviews_count: 320, is_flash_sale: true, is_all_time_low: true, created_at: now },
      { id: stableId(storeName, 4), title: `Anker 737 Power Bank 24000mAh 140W PowerCore 24K - ${storeName}`, title_ar: `باور بنك انكر 737 سريع الشحن بقوة 140 واط من ${storeName}`, store_id: stableId(storeName, 0), store_name: storeName, url: storeUrl, image_url: "https://images.unsplash.com/photo-1609592424368-2f1d2c6943e0?auto=format&fit=crop&w=600&q=80", current_price: 4299.0, original_price: 6800.0, discount_percent: 36.8, currency: "EGP", category: "Electronics", brand: "Anker", rating: 4.9, reviews_count: 510, is_flash_sale: false, is_all_time_low: true, created_at: now }
    ];
  }
  ...
  if (nameLow.includes("carrefour") || nameLow.includes("spinneys") || nameLow.includes("hyper") || nameLow.includes("gourmet") || category === "Supermarket") {
    return [
      { id: stableId(storeName, 1), title: `Ariel Platinum Liquid Laundry Detergent 3.3L - ${storeName}`, title_ar: `مسحوق غسيل سائل اريال بلاتينيوم 3.3 لتر اتوماتيك من ${storeName}`, store_id: stableId(storeName, 0), store_name: storeName, url: storeUrl, image_url: "https://images.unsplash.com/photo-1585670270608-b404fb0971f4?auto=format&fit=crop&w=600&q=80", current_price: 299.0, original_price: 460.0, discount_percent: 35.0, currency: "EGP", category: "Supermarket", brand: "Ariel", rating: 4.8, reviews_count: 1200, is_flash_sale: true, is_all_time_low: true, created_at: now },
      { id: stableId(storeName, 2), title: `Crystal Pure Corn Oil 3.5 Litre Big Family Pack - ${storeName}`, title_ar: `زيت ذرة كريستال نقي عبوة عائلية 3.5 لتر من ${storeName}`, store_id: stableId(storeName, 0), store_name: storeName, url: storeUrl, image_url: "https://images.unsplash.com/photo-1474979266404-7eaacbcd87c5?auto=format&fit=crop&w=600&q=80", current_price: 349.0, original_price: 490.0, discount_percent: 28.8, currency: "EGP", category: "Supermarket", brand: "Crystal", rating: 4.9, reviews_count: 980, is_flash_sale: false, is_all_time_low: true, created_at: now }
    ];
  }
  ...
  if (nameLow.includes("zara") || nameLow.includes("fashion") || nameLow.includes("clothes") || nameLow.includes("h&m") || category === "Fashion") {
    return [
      { id: stableId(storeName, 1), title: `Men's Water-Repellent Puffer Winter Jacket - ${storeName}`, title_ar: `جاكيت شتوي منفوخ مقاوم للماء للرجال من ${storeName}`, store_id: stableId(storeName, 0), store_name: storeName, url: storeUrl, image_url: "https://images.unsplash.com/photo-1544441893-675973e31985?auto=format&fit=crop&w=600&q=80", current_price: 1790.0, original_price: 3450.0, discount_percent: 48.1, currency: "EGP", category: "Fashion", brand: storeName, rating: 4.7, reviews_count: 240, is_flash_sale: true, is_all_time_low: true, created_at: now },
      { id: stableId(storeName, 2), title: `Classic Leather Chelsea Boots in Dark Brown - ${storeName}`, title_ar: `هاف بوت جلد تشيلسي كلاسيك بني غامق من ${storeName}`, store_id: stableId(storeName, 0), store_name: storeName, url: storeUrl, image_url: "https://images.unsplash.com/photo-1542291026-7eec264c27ff?auto=format&fit=crop&w=600&q=80", current_price: 1450.0, original_price: 2900.0, discount_percent: 50.0, currency: "EGP", category: "Fashion", brand: storeName, rating: 4.6, reviews_count: 180, is_flash_sale: true, is_all_time_low: true, created_at: now }
    ];
  }

  // General fallback
  return [
    { id: stableId(storeName, 1), title: `Smart 55 Inch UHD 4K Frameless Display - ${storeName}`, title_ar: `شاشة ذكية 55 بوصة بدقة 4K بدون حواف من ${storeName}`, store_id: stableId(storeName, 0), store_name: storeName, url: storeUrl, image_url: "https://images.unsplash.com/photo-1593359677879-a4bb92f829d1?auto=format&fit=crop&w=600&q=80", current_price: 12499.0, original_price: 19800.0, discount_percent: 36.9, currency: "EGP", category: "Electronics", brand: storeName, rating: 4.7, reviews_count: 310, is_flash_sale: true, is_all_time_low: true, created_at: now },
    { id: stableId(storeName, 2), title: `Digital Touch Air Fryer XXL 6.5L - ${storeName}`, title_ar: `قلاية هوائية رقمية تاتش سعة 6.5 لتر من ${storeName}`, store_id: stableId(storeName, 0), store_name: storeName, url: storeUrl, image_url: "https://images.unsplash.com/photo-1586201375761-83865001e31c?auto=format&fit=crop&w=600&q=80", current_price: 3450.0, original_price: 5800.0, discount_percent: 40.5, currency: "EGP", category: "Home & Kitchen", brand: storeName, rating: 4.8, reviews_count: 420, is_flash_sale: true, is_all_time_low: true, created_at: now },
    { id: stableId(storeName, 3), title: `Wireless Bluetooth Active Noise Canceling Headphones - ${storeName}`, title_ar: `سماعات لاسلكية بلوتوث عازلة للضوضاء من ${storeName}`, store_id: stableId(storeName, 0), store_name: storeName, url: storeUrl, image_url: "https://images.unsplash.com/photo-1505740420928-5e560c06d30e?auto=format&fit=crop&w=600&q=80", current_price: 1890.0, original_price: 3200.0, discount_percent: 40.9, currency: "EGP", category: "Electronics", brand: storeName, rating: 4.6, reviews_count: 190, is_flash_sale: false, is_all_time_low: true, created_at: now }
  ];
}


// ═══════════════════════════════════════════════════════════════════════════════
// SECTION 6: API Client — the public interface
// ═══════════════════════════════════════════════════════════════════════════════

export const api = {
  // 1. Deals Feed — refreshes timestamps on every call to simulate "live" data
  async getDeals(params = {}) {
    ...
    // ─── Client-side offline/static mode ───
    // Merge seed deals + custom deals, re-stamp timestamps for freshness
    let customDealsList = [];
    try {
    ...
    [...SEED_DEALS, ...customDealsList].forEach((d, idx) => {
      // Ensure every deal URL is a verified direct product page
      const directUrl = getLiveDealUrl(d);
      allDealsMap.set(d.id, {
        ...d,
        url: directUrl,
        // Re-stamp created_at to simulate fresh data on each load
        created_at: new Date(now - idx * 120000).toISOString()
      });
    });
    ...
    if (categoriesList.length > 0) {
      filtered = filtered.filter(d => {
        return categoriesList.some(cat => {
          ...
          // Fuzzy token match for custom categories
  ...
  async registerStore(storeData) {
    ...
    try {
      ...
      const updatedDeals = [
        ...generatedDeals,
        ...currentDeals.filter(d => (d.store_name || "").toLowerCase() !== cleanName.toLowerCase())
      ];
